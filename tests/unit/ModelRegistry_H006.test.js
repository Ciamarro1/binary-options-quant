...
describe('Model Registry Governance: MODEL_H006 Manifest Audit', () => {
  const manifestPath = path.join(__dirname, '..', '..', 'artifacts', 'model_registry', 'MODEL_H006_MANIFEST.json');

  test('manifest file exists and parses as valid JSON', () => {
    expect(fs.existsSync(manifestPath)).toBe(true);
    const content = fs.readFileSync(manifestPath, 'utf-8');
    const parsed = JSON.parse(content);
    expect(parsed.modelId).toBe('MODEL_H006_XAUXAG_TREND_ZREVERT');
  ...
  test('enforces Live Production Gate lock until Level 2 Fidelity Audit', () => {
    const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf-8'));
    expect(manifest.deploymentGates.promotionToLiveProduction).toBe('BLOCKED_PENDING_LEVEL_2_FIDELITY');
    expect(manifest.deploymentGates.promotionToPaperDemo).toBe('AUTHORIZED');
  });
});
