...
const RatioZScoreModel = require('../models/RatioZScoreModel');
...
class H005Runner {
  constructor(observations, options = {}) {
    ...
    this.modelClass = options.modelClass || RatioZScoreModel;
  }
  ...
  /**
   * Replays model over a specified index slice [startIndex, endIndex).
   */
  runSlice(startIndex, endIndex) {
    const model = new this.modelClass(this.lookback, this.threshold, this.expiryBars * 60);
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      // 1. Predict BEFORE update (Causal Invariant)
      const signal = model.predict(candle);

      if (signal && (signal.direction === 'CALL' || signal.direction === 'PUT')) {
        // Enforce boundary: resolution must occur strictly within the slice
      ...
      // 2. Update AFTER predict (Causal Invariant)
      model.update(candle);
    }
    ...
    const wilsonCI = H005Runner.computeWilsonCI(totalWins, totalNonPush);
    ...
    for (const t of trades) {
      ...
      const predicted = 0.5319; // reference hurdle
  ...
  /**
   * Executes partitioned Walk-Forward replay across declared IS and OOS boundaries.
   */
