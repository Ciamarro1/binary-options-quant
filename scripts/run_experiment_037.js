...
/**
 * scripts/run_experiment_037.js
 * 
 * Execution of HYPOTHESIS_007 Blind Walk-Forward Replay on DATASET_XAUXAG_001.
 * Exhaustion / Rejection Wick Confirmed Ratio Mean Reversion.
 */

const fs = require('fs');
const path = require('path');
const DatasetLoader = require('../src/data/DatasetLoader');
const H007Runner = require('../src/strategy/runners/H007Runner');
...
const REPORT_OUTPUT = path.join(__dirname, '..', 'research', 'reports', 'VALIDATION_REPORT_007.json');

async function main() {
  console.log('================================================================');
  console.log('EXPERIMENT 037: BLIND WALK-FORWARD REPLAY (HYPOTHESIS_007)');
  console.log('Hypothesis: Rejection Wick Confirmed Intraday Ratio Mean Reversion');
  ...
  console.log('\n[RUN] Executing Standard HYPOTHESIS_007 Model...');
  const runner = new H007Runner(dataset.observations, {
    ...
    expiryBars: 15,
    payoutRate: 0.88,
    modelClass: WickConfirmedRatioModel
  });
  ...
  const reversedRunner = new H007Runner(dataset.observations, {
    intradayLookback: 120,
    macroLookback: 1440,
    zThreshold: 2.0,
    minWickRatio: 0.35,
    expiryBars: 15,
    payoutRate: 0.88,
    modelClass: ReversedWickConfirmedModel
  });
  ...
  console.log('\n================================================================');
  console.log('OUT-OF-SAMPLE (BLIND EVALUATION) RESULTS — HYPOTHESIS_007');
  console.log('================================================================');
  ...
  const report = {
    experimentId: 'EXP_037_XAUXAG_WICK_CONFIRMED_REVERT_001',
    hypothesisId: 'HYPOTHESIS_007',
    asset: 'XAUXAG',
    ...
    outOfSampleResults: {
      ...
      callWinRate: oos.callStats.winRate,
      putWinRate: oos.putStats.winRate,
      directionalImbalance: oos.directionalImbalance,
      brierScore: oos.brierScore,
      breakevenRate: oos.breakevenRate,
      surplusOverBreakevenBps: Math.round((oos.wilsonCI.lower - oos.breakevenRate) * 10000),
      reversedControlWinRate: revOos.winRate
    },
    evaluation: {
      meetsSampleSize: oos.resolvedTrades >= 100,
      clearsBreakevenHurdle: oos.wilsonCI.lower > oos.breakevenRate,
      directionalSymmetryAcceptable: oos.directionalImbalance < 0.15,
      reversedControlSubverted: revOos.winRate < 0.50
    }
  };

  fs.writeFileSync(REPORT_OUTPUT, JSON.stringify(report, null, 2), 'utf8');
  console.log(`[SAVED] Validation report written to ${REPORT_OUTPUT}`);
}
